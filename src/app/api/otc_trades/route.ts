...
const otcTradesSchema: z.ZodType<OtcTrade> = z.object({
  ...
  iv: z.string().refine((val) => {
    try {
      ...
      return decoded.length === 12 // Must be exactly 12 bytes as generated by crypto.getRandomValues(new Uint8Array(12))
    } catch (err) {
      return false
    }
  }, "Invalid IV format - must be a valid base64url string of exactly 12 bytes"),
})
