...
const ActivityDock = () => {
  ...
  return (
    <div className="flex flex-col justify-end w-full mt-auto grow pb-32">
      {dockItems.map((item) => (
        <DockItemCard key={item.id} item={item} onRemove={removeDockItem} />
      ))}
    </div>
  )
}
...
function DockItemCard({
  item,
  onRemove,
}: { item: DockItem; onRemove: (id: string) => void }) {
  const { updateDockItem } = useActivityDock()

  const handleUpdateItem = useCallback(
    (updates: Partial<DockItemBase>) => {
      updateDockItem(item.id, updates)
    },
    [item.id, updateDockItem]
  )
  ...
  return (
    <div className="bg-white rounded-2xl p-3 overflow-hidden">
      ...
      {isCustom ? (
        <div className="mt-4">{item.renderContent(handleUpdateItem)}</div>
      ) : (
