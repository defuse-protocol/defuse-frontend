...
describe("validationRecipientAddress", () => {
  ...
  describe("Zcash network", () => {
    ...
    it("accepts a valid transparent address", async () => {
      const result = await validationRecipientAddress(
    ...
    it("accepts a valid UA address", async () => {
      const result = await validationRecipientAddress(validUAAddress, "zcash")
      expect(result.isOk()).toBe(true)
    })

    it("rejects a transparent address with invalid characters", async () => {
      const result = await validationRecipientAddress(
    ...
    it("rejects a ua address with invalid characters", async () => {
      const result = await validationRecipientAddress(invalidUA, "zcash")
      expect(result.isErr()).toBe(true)
      expect(result.unwrapErr()).toBe("ADDRESS_INVALID")
    })
