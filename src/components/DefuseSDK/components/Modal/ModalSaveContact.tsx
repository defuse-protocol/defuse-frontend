...
type ModalSaveContactProps = {
  ...
  address: string
  /** The network for the contact (pre-filled, read-only). Can be a standard chain or "near_intents". */
...
const ModalSaveContact = ({
  ...
  const blockchain: ContactBlockchain =
    network === "near_intents" ? "near_intents" : assetNetworkAdapter[network]
  const { chainIcon, chainName } = getContactChainInfo(blockchain)
